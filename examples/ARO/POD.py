...
def train_pod_mlp(model, loader, graph, epochs=500, lr=1e-3, device="xpu"):
    model.to(device)
    ...
    for ep in range(epochs):
        ...
        if ep % 5 == 0 and ep%50 !=0:
            ...
            original = torch.tensor(fields[random_number], dtype=torch.float32).view(-1)
            graph.plot_pos_field(p_pred, azim=180, elev=0, s=0.05,filename=f"/lus/flare/projects/Prob_AI/kanadsen/myrepos/dgn4cfd_tum/examples/ARO/outputs_mach/mach_pod_mlp_pred_{ep}.png")
            graph.plot_pos_field(original, azim=180, elev=0, s=0.05,filename=f"/lus/flare/projects/Prob_AI/kanadsen/myrepos/dgn4cfd_tum/examples/ARO/outputs_mach/mach_original_{ep}.png")
...
# Train MLP
pod_dataset = PODDataset(params, coeffs)
loader = DataLoader(pod_dataset, batch_size=32, shuffle=True)
...
train_pod_mlp(model, loader, graph, epochs=500)
